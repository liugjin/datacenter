...
define ['../base-directive','text!./style.css', 'text!./view.html', 'underscore', "moment"], (base, css, view, _, moment) ->
  class EquipLineorbarlessDirective extends base.BaseDirective
    constructor: ($timeout, $window, $compile, $routeParams, commonService)->
      @id = "equip-lineorbarless"
...
#          scope.controller.prompt '错误', '请选择1-3个信号!!'
      ...
      scope.equipment.loadSignals null, (err, signallists) =>
        signallists=_.filter(signallists,(signal)=>signal.model.signal=="subharmonic"||signal.model.signal=="totalharmonic")
        ...
        if signals and signals.value
          scope.signalList = _.map(JSON.parse(signals.value), (d, i) =>
            ...
            return { signal: d.signal, name: _signal?.model.name, check: true}
          )
        else
  ...
  exports =
    EquipLineorbarlessDirective: EquipLineorbarlessDirective