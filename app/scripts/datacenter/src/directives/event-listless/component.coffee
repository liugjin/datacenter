...
define ['../base-directive','text!./style.css', 'text!./view.html', 'lodash', "moment"], (base, css, view, _, moment) ->
  class EventListlessDirective extends base.BaseDirective
    ...
    show: (scope, element, attrs) =>
      ...
      window.debugR = scope
      scope.subStationEquipIds?.dispose()
      scope.subStationEquipIds = @commonService.subscribeEventBus 'equipmentId', (d)=>
        if d
          scope.eventsArray=_.filter(scope.eventsArray,(event)=>event.equipment ==d.message.equipmentId.equipment)
          scope.$applyAsync()
...
# match events belong to this station and its children station
  ...
  exports =
    EventListlessDirective: EventListlessDirective