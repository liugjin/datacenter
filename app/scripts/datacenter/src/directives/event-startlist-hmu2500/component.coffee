...
define ['../base-directive','text!./style.css', 'text!./view.html', 'underscore', "moment"], (base, css, view, _, moment) ->
  class EventStartlistHmu2500Directive extends base.BaseDirective
    constructor: ($timeout, $window, $compile, $routeParams, commonService)->
      @id = "event-startlist-hmu2500"
    ...
    show: (scope, element, attrs) =>
      ...
      eventSeverity = scope.project?.dictionary?.eventseverities?.items
      ...
      #一键结束
      scope.endKeyEvents = (station)=>

        staData = _.map (@commonService.loadStationChildren station, true),(mps)=> return mps.model.station
        filterStation = _.filter scope.eventsArray,(evt)=> evt.station in staData

        if filterStation.length == 0
          @display "", "当前机房及其子机房暂无告警！", 1200
          return false
        ...
        scope.prompt title, message, (ok, comment) =>
          ...
          confirmEndKeyEvent station, comment, true, filterStation
        ,true


      confirmEndKeyEvent = ( station, comment, forceToEnd, filterStation )=>
        _.each filterStation,(event)=>
          data = {
            _id: event._id
            user: event.user
            project: event.project
      ...
      subscribeStationEvent=(stationId)=>
        ...
        stationIdArray.forEach (id)=>
          ...
          eventSubscription=@commonService.eventLiveSession.subscribeValues filter,(err,msg) =>
            ...
            processEvent(msg.message)

            # if msg.message.endTime == undefined
            #   processEvent(msg)
      ...
      processEvent=(message) ->
        ...
        delete scope.events[key] if event.endTime
      ...
      scope.queryEventRecords= (event, periodType, page = 1, pageItems = 20) =>
        ...
        scope.eventRecords = []
        # console.log event
        # console.log moment(period.startTime).format("YYYY-MM-DD hh:mm:ss"), moment(period.endTime).format("YYYY-MM-DD hh:mm:ss")
        startTime = moment(new Date).format("YYYY-01-01 00:00:00")
        endTime = moment(new Date).format("YYYY-MM-DD hh:mm:ss")
        @commonService.queryEventRecords event,startTime,endTime,null, (err, records) =>
          if records
