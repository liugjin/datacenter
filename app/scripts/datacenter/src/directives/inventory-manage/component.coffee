    ...
    show: ($scope, element, attrs) =>
      ...
      selectType = (type, callback, refresh) =>
        ...
        getStationEquipment $scope.station, (equips) =>
          ...
          $scope.$applyAsync()
          for equip in equips
            ownerobj=(_.find $scope.userMsg ,(userobj)=>userobj.user == equip.model.owner)
            equip.model.owner=ownerobj?.name
...
#        console.log equipment
      ...
      setEquipmentData = () =>
        _.map $scope.equipment.properties.items, (item) =>
          ...
          console.log $scope.equipment.sampleUnits
...
#        这句话会导致复制一次设备，properties.items的元素翻倍
...
#            spliceStatusFormat($scope.equipment)
      ...
      $scope.saveEquipmentGroups = () =>
        ...
        tmpProps = []
        for prop in $scope.equipment.properties.items
          tmpProps.push {id:prop.model.property,value:prop.value}
      ...
      $scope.editData = (equipment) =>
        ...
        if equipment
          $scope.equipment = equipment
          $scope.equipment.model?.sampleVals = angular.copy equipment.sampleUnits
      ...
      $scope.stationCheck = () =>
        if $scope.equipment.model.station
          $scope.saveEquipment()
      #     处理设备里的状态属性，
      spliceStatusFormat = (equip) =>
        strs = {}
        if equip
          for i in equip.properties?.items
            if i.model?.dataType == 'enum'
              str1 = i.model?.format.split(',')
              for j in str1
                key = j.split(':')[0]
                value = j.split(':')[1]
                strs[key] = value
