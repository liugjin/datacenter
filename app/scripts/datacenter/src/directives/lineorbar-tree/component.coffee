...
define ['../base-directive','text!./style.css', 'text!./view.html', 'underscore', "moment"], (base, css, view, _, moment) ->
  class LineorbarTreeDirective extends base.BaseDirective
...
#          scope.controller.prompt '错误', '请选择1-3个信号!!'
      ...
      getHistoryData = (message) =>
        ...
        @commonService.reportingService.querySignalStatistics { filter: filter, fileds: null,paging:null,sorting:null },(err,statistic) =>
          if typeof(statistic) is "object"
            ...
            aaa=_.sortBy _chartValues,(item)->item.index
            sum = 0
            scope.chartValues = aaa
          else
            cleanData()
      ...
      scope.subEventService?.dispose()
      scope.subEventService = @commonService.subscribeEventBus "selectEquip",(msg)=>
        messageMsg = msg.message
        if messageMsg.level isnt "station"
          return if _.isEmpty(scope.project.equipments)

          #设备
          findEquip = _.find scope.project.equipments,(eq)=> (eq.model.station is messageMsg.station ) and ( eq.model.equipment is messageMsg.id )
          return if !findEquip
