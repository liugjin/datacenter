  ...
  NotificationDirective = (function(_super) {
    ...
    NotificationDirective.prototype.show = function(scope, element, attrs) {
      ...
      scope.save = (function(_this) {
        return function() {
          var contentstr, contenttypestr, filterEquipments, filterStations, filterTypes, filterUsers, filtereventSeverities, name, save_api, title, users;
          if (scope.user_all_select) {
            users = ["_all"];
          } else {
          ...
          switch (scope.cate) {
            ...
            case 'sms':
              name = "sms-1";
              title = "短信告警通知";
              contentstr = '//js if($event.phase == "start"){ return $event.stationName + "--" + $event.equipmentName + "--"  + $event.title; }else if($event.phase == "end"){ return $event.stationName + "--" + $event.equipmentName + "--"  + $event.title +  "。恢复正常"; }';
