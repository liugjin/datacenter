...
define ["jquery",'../base-directive','text!./style.css', 'text!./view.html', 'underscore', "moment",'angularGrid','gl-datepicker'], ($,base, css, view, _, moment,agGrid,gl) ->
  class ReportAlarmRecordsDirective extends base.BaseDirective
...
#        gridOptions.api.exportDataAsCsv({fileName:reportName, allColumns: true, skipGroups:true})
      ...
      queryAlarms =(page= 1 ,pageItems = scope.parameters.pageItems)=>
...
#          return console.log('err:',err) if err
          ...
          tmprecords = _.map records, (record) =>
            ...
            if _.isEmpty record.endTime
              record.continuedTime = @calTimes(moment(nowTime).diff(moment(moment(record.startTime).format("YYYY-MM-DD HH:mm:ss"), 'YYYY-MM-DD HH:mm:ss'), 'seconds'))
              record.startTime = moment(record.startTime).format('YYYY-MM-DD HH:mm:ss')
            else
              record.continuedTime = @calTimes(moment(record.endTime).diff(moment(moment(record.startTime).format("YYYY-MM-DD HH:mm:ss"), 'YYYY-MM-DD HH:mm:ss'), 'seconds'))
              record.startTime = moment(record.startTime).format('YYYY-MM-DD HH:mm:ss')
      ...
      querySignal = (page= 1 ,pageItems = scope.parameters.pageItems)=>
        return if checkFilter()
