...
#      console.log("selectedAction",@)
    ...
    unpackAction:()->
      for child in @showingObject.children
        t = child?.$thing
        if t?.userData?.unpackPositionOffset
          if !t?.unpackPositionOffset
            offset = t?.userData?.unpackPositionOffset
          ...
          t.position.copy t.unpackOffsetPosition
    ...
    fitParameters:()->
      ...
      scale = @scale.clone()
      console.log "position:", position if @name is "micromodule-10"
      console.log "rotation:", rotation if @name is "micromodule-10"
    ...
    changeToNormal:()->
      ...
      for g in Array.from(old.children)
        if g?.type is "Group"
          switch g.$thing?.showingType
            when "normal"
              if @showingObject.uuid != g.$thing.normal.uuid
                @showingObject.add(g.$thing.normal)
            when "wireframe"
              if @showingObject.uuid != g.$thing.wireframe.uuid
                @showingObject.add(g.$thing.wireframe)
            when "capacity"
              if @showingObject.uuid != g.$thing.capacity.uuid
                @showingObject.add(g.$thing.capacity)
    ...
    changeToWireframe:()->
      ...
      for g in Array.from(old.children)
        if g?.type is "Group" and g.$thing?.showingObject and @showingObject.uuid != g.$thing.showingObject.uuid
          @showingObject.add(g.$thing.showingObject)
