...
define ['../base-directive','text!./style.css', 'text!./view.html', 'underscore', "moment"], (base, css, view, _, moment) ->
  class StationEnvironmentChongqingDirective extends base.BaseDirective
    constructor: ($timeout, $window, $compile, $routeParams, commonService)->
      @id = "station-environment-chongqing"
    ...
    getStationEnvironment: (scope, station) ->
      ...
      # @commonService.loadEquipmentsByType(station, "environmental", (err, equipments)=>
        ...
        scope.smokes = _.filter(equipments, (equipment)->equipment.model.template == "smoke_template")
        if scope.smokes?.length >2
          scope.smokes=scope.smokes.slice(1)
        scope.equipments = _.filter(equipments, (equipment)->equipment.model.template == "temperature_humidity_template")
        _.each(scope.smokes, (smoke)=>
