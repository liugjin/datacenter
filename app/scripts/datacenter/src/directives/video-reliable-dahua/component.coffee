...
define ['../base-directive','text!./style.css', 'text!./view.html', 'underscore', "moment"], (base, css, view, _, moment) ->
  class VideoReliableDahuaDirective extends base.BaseDirective
    constructor: ($timeout, $window, $compile, $routeParams, commonService)->
      @id = "video-reliable-dahua"
...
#      初始化
      ...
      scope.selectVideo= (video, index, playstatus=false) =>
        ...
        rtspstr=""
#        rtsp= rtsp://admin:admin123@192.168.10.251/cam/realmonitor?channel=1&subtype=1
        ...
        ipstr=somethingstr.split(':')?[1].split('@')?[1].split('/')?[0]

#              //登录当前设备
        scope.ret = WebVideoCtrl.login ipstr,37777,userstr,pwdstr,0
        if(scope.ret==0)
          console.log("login successed")
#         WebVideoCtrlconnectRealVideoEx(获得通道号,获得码流类型,窗口号)
          WebVideoCtrl.connectRealVideoEx(0,2,scope.g_iWndIndex);
        else
