...
define ['clc.foundation',
  ...
  './datacenter-service',
  './systemInfo-service',
  './configurationSetup-service',
  './backupInfo-service',
  './getVersion-service',
  './timeManage-service'
], (base, web, cs, sis,css, bis, gvs, tms) ->
  class ServiceRegister extends base.ServiceRegisterBase
    ...
    createService: (name) ->
      switch name
        ...
        when 'SystemInfoService'
          service = new sis.SystemInfoService @getOptions('services')
        when 'ConfigurationSetuoService'
          service = new css.ConfigurationSetuoService @getOptions('services')
